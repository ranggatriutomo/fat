...
// var_dump($data['no_pc']);
  ...
  for($i = 0; $i < count($turaian) ; $i++){
    $queryInsert = "INSERT INTO pc_fat (no_pc,no_intern,nama_pemohon, divisi, perihal, tgl_pc, kode_barang, uraian_pc, jumlah_pc, satuan_pc, harga_pc, total_pc, status_req, norek) 
    VALUES ('". $data['no_pc'] ."','". $data['no_intern'] ."','". $data['nama_pemohon'] ."','". $data['divisi'] ."','". $data['perihal'] ."','". $data['tgl_pc'] ."','". $tkode[$i] ."','". $turaian[$i] ."','". $tjumlah[$i] ."','". $tsatuan[$i] ."','". $tharga[$i] ."','". $ttotal[$i] ."','". $data['status_req'] ."','". $data['norek'] ."')" ;
